        ...
        for dirpath, dirs, files in os.walk(folderDir):
            for filename in files:
                ...
                if ext in ["mp4", "mkv", "mov"] and docChoice in [1, 3]:
                    if self.mongoInstance.checkFileInDB(filename=filename, filetype="video"):
                        self.videoSkipCount += 1
                        print(self.videoSkipCount,
                              "Video Already Present", filename)
                        continue
                    ...
                    if self.mongoInstance.checkFileInDB(filename=filename, filetype="photo"):
                        ...
                        print(self.photoFileCount,
                              "Photo Already Present", filename)
                        continue
